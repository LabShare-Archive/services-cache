 ...
 * 
 */    
  saveJSONForCache(key,data,duration, callback)
    {
        try 
        {
            key = this.options.createKey(key);
            let newData = this.options.setJsonValue(data);
                //creating the object in redis
...
getJSONObject(key, callback) {
	try {
            ...
            {
                return callback(error,this.options.formatJsonValue(reply));
            });
	} catch(error) {
