...
class Cache extends EventEmitter {
    ...
    constructor(redis, maxTime, ...isPubSub) {
        ...
        if (this.isPubSub) {
            ...
            this.client.on("message_buffer", (channel, message) => {
                this.emit("message_buffer", channel, this.formatObjectValue(message));
            });
            this.client.on("pmessage_buffer", (pattern, channel, message) => {
                this.emit("pmessage_buffer", pattern, channel, this.formatObjectValue(message));
            });
        }
    };
    /**
    ...
    deleteCatalog(listKey, callback) {
        ...
        let data = q.async(function* () {
            ...
            return yield q.ninvoke(self.client.multi(idList), "exec")
        });
        return data().nodeify(callback);
    };
    /**
  ...
  */
    deleteDataByScan(prefix, callback) {
        let self = this;
        let idList = [];
        prefix = self.createKey(prefix);
        let cursorInfo = { cursor: null, listItems: [] };;
        let data = q.async(function* () {
            while (cursorInfo.cursor != 0) {
                let data = yield q.ninvoke(self.client, 'send_command', 'scan', [`${cursorInfo.cursor || 0}`, 'MATCH', `${prefix}*`, 'COUNT', '10']);
                cursorInfo.cursor = data[0];
                cursorInfo.listItems = data[1];
                for (let i = 0; i < cursorInfo.listItems.length; i++) {
                    idList.push(['del', cursorInfo.listItems[i]]);
                }
            }
            return yield q.ninvoke(self.client.multi(idList), "exec")
        });
    ...
    */
    ...
    /**
     * @description Method for subscribe to the Redis pubsub.
     * @param {string} [pattern] - Pattern to subscribe
     */
    psubscribe(pattern) {
    ...
    /**
     * @description Clone object method.
     */
    clone() {
        return _.clone(this);
    }
