 ...
 * {number} options.configuration.maxTime - The default max time of each object for store in cache. Default : null
 * @constructor
 */
class Cache 
{
    ...
    get redisClient() {
     ...
     /**
     * @description Property for get the internal redis client.
     */
    set redisClient(value) {
        this.client = value;
    }
     /**
     * @description Property for set the create key method.
     */
    set createKey(fn) {
        this.options.createKey = fn;
    }
     /**
     * @description Property for set the setvalue method (works for single values).
     */
    set setValue(fn) {
        this.options.setValue = fn;
    }
     /**
     * @description Property for set the setObjectvalue method (works for objects).
     */   
    set setObjectValue(fn) {
        this.options.setObjectValue = fn;
    }
    /**
     * @description Property for set the formatvalue method (works for single values).
     */ 
     set formatValue(fn) {
        this.options.formatValue = fn;
    }
    /**
     * @description Property for set the formatObjectvalue method (works for objects).
     */      
 ...
 * @param {int} [duration] - The duration in seconds, -1 for infinite duration.
 * @param {object} [data] - Optional data for the catalog.
 * 
 * */ 
    addToCatalog(listKey,itemID,duration,...data)
    {
        try 
        {
            listKey = this.options.createKey(listKey); 
            let newID = this.options.createKey(itemID);
            let newData =(data)? this.options.formatValue(data):null;
            this.client.sadd(listKey,newID,newData);
	...
	try {
        ...
        this.client.smembers(listKey,(error,members)=>
        {
         for(let i = 0 ; i < members.length;i++)
             data.push(this.options.formatValue(members[i]));
...
deleteCatalog(listKey , callback) {
    ...
    let data = q.async(function*() {
	try {
        let listItems = yield methods.smembers(listKey);
    
        for( let i = 0 ; i < listItems.length ; i++)
        {
            let members  = yield methods.zrange(listItems[i],0,-1);
...
/**
 * @description Private method for set the expiracy of the objects.
 * @param {object} [client] - The Redis' connection client.
 * @param {string} [key] - The unique ID of the object.
 * @param {int} [duration] - The duration in seconds, -1 for infinite duration.
 */   
