...
class Cache {
    ...
    saveObject(key, data, duration, callback) {
        ...
        let newData = this.setObjectValue(data);
        this._hsetTypeSave(this.client, key, newData)
            .then((response) => {
    ...
    getObject(key, callback) {
        ...
        try {
            key = this.createKey(key);
            q.ninvoke(this.client, 'hgetall', key)
                .then((reply) => {
                    return deferred.resolve(this.formatObjectValue(reply));
                }, (error) => {
                    return deferred.reject(error);
                });
        } catch (error) {
    ...
    set(key, data, duration, callback) {
        let deferred = q.defer();
        key = this.createKey(key);
        q.ninvoke(this.client, 'set', key, this.setValue(data))
            .then((response) => {
                this._setExpiracy(this.client, key, duration);
                return deferred.resolve(response);
            }, (error) => {
    ...
    get(key, callback) {
        let deferred = q.defer();
        key = this.createKey(key);
        q.ninvoke(this.client, "get", key)
            .then((response) => {
                return deferred.resolve(this.formatValue(response));
            }, (error) => {
                return deferred.reject(error)
            });
        return deferred.promise.nodeify(callback);
    ...
    addToCatalog(listKey, itemID, duration, callback) {
        ...
        q.ninvoke(this.client, "sadd", listKey, newID)
            .then((data) => {
    ...
    getAllFromCatalog(listKey, callback) {
        ...
        listKey = this.createKey(listKey);
        let data = new Array();
        q.ninvoke(this.client, "smembers", listKey).then((members) => {
            for (let i = 0; i < members.length; i++)
