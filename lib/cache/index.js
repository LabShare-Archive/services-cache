...
class Cache extends EventEmitter {
    ...
    incr(listKey, seed, duration, callback) {
        ...
        let data = q.async(function* () {
            if (!_.isNil(seed))
                yield q.ninvoke(self.client, 'send_command', 'set', [listKey, seed]);
            return yield q.ninvoke(self.client, 'send_command', 'incr', [listKey]);
    ...
    getIncr(listKey, callback) {
        ...
        listKey = self.createKey(listKey);
        let data = q.async(function* () {
            return yield q.ninvoke(self.client, 'send_command', 'get', [listKey]);
        });
        return data().then((response) => {
            return this.formatValue(response);
        }).nodeify(callback);
