...
class Cache {
    ...
    constructor(redisConfiguration, maxTime) {
        this.maxTime = null;
        this.client = null;
        assert(_.isObject(redisConfiguration) && redisConfiguration.host, '`redisConfiguration` must be a valid connection string');
        if (maxTime) {
     ...
     * 
     * */
    addToCatalog(listKey, itemID) {
        try {
            listKey = this.createKey(listKey);
            let newID = this.createKey(itemID);
            this.client.sadd(listKey, newID);
            this._setExpiracy(this.client, listKey, null);
        } catch (error) {
     ...
     * 
     * */
    addToCatalogAsync(listKey, itemID, callback) {
        try {
            listKey = this.createKey(listKey);
            let newID = this.createKey(itemID);
    ...
    deleteCatalog(listKey, callback) {
        ...
        let methods = {
            ...
            zrange: q.nbind(self.client.zrange, self.client),
            command: q.nbind(self.client.send_command, self.client)
        };
        listKey = self.createKey(listKey);
        let data = q.async(function* () {
            try {
                ...
                for (let i = 0; i < listItems.length; i++) {
                    let type = yield methods.command('TYPE', [listItems[i]]);
                    if (type === 'zset') {
