...
class Middleware {
  constructor(
    ...
    if (logger)
      assert(
        ...
        '`logger` must define an `error` function',
      );
    this.logger = logger;
    assert.ok(
      _.isObject(redis) && redis.host,
      '`redis` must be a valid connection string',
    );
    if (maxTime)
    ...
    return (req, res, next) => {
      try {
        ...
        req.cacheHelper.cacheData = (
          ...
          self._cacheData(
            self._createKeyForExtendedMethods(key),
            data,
            duration,
            catalog,
            callback,
        ...
        (req.cacheHelper.getfromCache = (key, callback) => {
          ...
          (req.cacheHelper.deleteDataByScan = (key, callback) => {
            try {
              ...
              self.cacheClient.deleteDataByScan(
                self._createKeyForExtendedMethods(key),
                callback,
              );
            } catch (error) {
        ...
        req.cacheHelper.refresh = (catalog, callback) => {
          try {
            ...
            self.cacheClient.deleteCatalog(
              [self.prefix].concat(catalog),
              callback,
            );
          } catch (error) {
            self._handleError(error);
        ...
        error.message =
          'Failed to get the enable services-cache: ' + error.message;
        self._handleError(error);
        next();
      }
      next();
  ...
  _cacheData(key, data, duration, catalog, callback) {
    try {
      ...
      self.cacheClient.saveObject(key, data, duration, (error, data) => {
        ...
        if (catalog) self._addToCatalog(catalog, key);
  ...
  _addToCatalog(catalog, name) {
    try {
      ...
      this.cacheClient.addToCatalog(
        [this.prefix].concat(catalog),
        name,
        this.catalogDuration,
      );
    } catch (error) {
      this._handleError(error);
    }
  ...
  _createKeyForExtendedMethods(key) {
    let newKey = [];
    if (!_.isEmpty(this.prefix)) newKey.push(this.prefix);
