 ...
 */
    ...
    _cacheResponse (key,data,duration)
    {
        try
        {
            this.cacheClient.saveJSONForCache(key,data,duration,(error,data)=>{
                if(error)
                {
    ...
    _getfromCache(key,callback)
    {
        try
        {
            this.cacheClient.getJSONObject(key,(error,data)=>{
   ...
   _generateResponseForCache(req,res,next, key , duration)
    {
        ...
        self._cacheResponse(key,{status: res.statusCode,headers:res._headers,data:contentdata,encoding:encoding},duration);};
    ...
    getMiddleware(duration,middlewareHelper)
    {
      ...
      return (req,res,next)=>{
          try
          {
            ...
            if(this._isNotAllowedForCache(req))
                next();
     ...
     _isNotAllowedForCache(req)
    {
        if(req)
            return (this.notAllowedCacheRoutes.has(req.originalUrl));
        else return false;
