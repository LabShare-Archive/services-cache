...
class Middleware 
{
    ...
    set providerClient(value) {
        this.cacheClient = value;
    }
    constructor(options={})
    {
...
/**
 * @description Assigns all the connection and configuration information for the Cache provider for LabShare
 *
 ...
 */
    initialize() {
        try {
            ...
            this.cacheClient.setObjectValue = ((value)=>{return JSON.stringify(value.data);});
            this.cacheClient.formatObjectValue = ((value)=>{return JSON.parse(value);})   
        } catch (error) {
 ...
 /**
 * @description adds the request url for not use cache.
 ...
 */  
 ...
 /**
 * @description adds the request url to a catalog.
* @param {string | array} [name] - The catalog's name. 
 * @param {object} [req] - The express' request object. 
 */  
    addToCatalog(name,req)
    {
        try
        {
            this.cacheProvider.addToCatalog(name,req.originalUrl);
        }catch(error)
        {
 ...
 /**
 * @description Deletes the catalog and it's contents.
* @param {string | array} [name] - The catalog's name. 
 * @param {object} [req] - The express' request object. 
 */  
    deleteCatalog(name,callback)
    {
        try
        {
            this.cacheProvider.deleteCatalog(name,callback);
        }catch(error)
        {
 ...
 * @param {function} [middlewareHelper] - The function for Route's Middleware. 
 */    
