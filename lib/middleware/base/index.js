 ...
 * {Object} redis - Redis API configuration.
 ...
 */
class Middleware {
    constructor(redis, maxTime,prefix='',catalogDuration=-1, logger=null) {
        if (logger)
        assert(_.isFunction(logger.error), '`options.logger` must define an `error` function');
        this.logger = logger;    
