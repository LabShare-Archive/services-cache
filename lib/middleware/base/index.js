...
class Middleware {
                    ...
                    self._getfromCache(key, (error, data) => {
                        try {
                            if (error)
                                throw error;
                            if (data)
                                return self._createCachedResponse(req, res, data);
     ...
     */
    _createCachedResponse(request, response, cacheObj) {
        let data, innerData;
        let headers = cacheObj.headers;
        //checking the inner data of the object - for buffers , inner data will wrap the contents of data.
        innerData = cacheObj.data;
        if (innerData.type === 'Buffer') {
            data = new Buffer(innerData);
        } else {
            // if not post the regular data
            data = innerData;
        }
        _.set(headers, 'services-cache', 'on');
