...
class Middleware {
    constructor(redisConfiguration, maxTime, options) {
        ...
        assert.ok(_.isObject(redisConfiguration) && redisConfiguration.host, '`redisConfiguration` must be a valid connection string');
        if (maxTime)
            assert.ok(_.isNumber(maxTime), 'maxTime` needs to be a valid number');
        this.cacheClient = new cacheProvider(redisConfiguration, maxTime);
