 ...
 */
class Middleware {
    constructor(redisConfiguration, options) {
        this.cacheClient = new cacheProvider(redisConfiguration, null, options);
    ...
    getMiddleware(duration) {
            return (req, res, next) => {
                try {
                    ...
                    //refresh method if is available
                    if (req.method != 'GET') {
                        return this._refreshCache(req, res, next);
                    }
                    //attach cache method
    ...
    /**
     * @description Private method for refresh the cache.
     * @param {object} [req] - The request object.
     * @param {object} [res] - The response object.
     * @param {function} [next] - Next method to call.
     */
    _refreshCache(req, res, next) {
        let self = this;
        res._end = res.end;
        res.end = function (content, encoding) {
            if (res) {
                if (!res.req.ignoreCache && res.req.catalog) {
